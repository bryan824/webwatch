...
use crate::{
    config::{CheckOutcome, CheckRun, EngineUsed, Target, TargetStatus},
...
pub(crate) const CREATE_TABLES: [&str; 3] = [
    "CREATE TABLE IF NOT EXISTS targets (id TEXT PRIMARY KEY, name TEXT NOT NULL, url TEXT NOT NULL, enabled INTEGER NOT NULL DEFAULT 1, interval_secs INTEGER, render_json TEXT NOT NULL DEFAULT '{}', conditions_json TEXT NOT NULL, updated_at TEXT NOT NULL)",
...
pub(crate) const STATUS_SQL: &str = "SELECT t.id, t.name, t.url, t.enabled, t.render_json, s.matched, s.engine_used, s.price_cents, s.evidence_json, s.condition_results_json, s.last_success_at, s.last_error_at, s.last_error, s.last_alert_at FROM targets t LEFT JOIN target_state s ON s.target_id = t.id ORDER BY t.id";
...
pub(crate) struct StatusParts {
    ...
    pub(crate) render_json: Option<String>,
...
pub(crate) struct CheckRunParts {
    pub(crate) checked_at: String,
    pub(crate) matched: Option<i64>,
    pub(crate) engine_used: Option<String>,
    pub(crate) price_cents: Option<i64>,
    pub(crate) evidence_json: String,
    pub(crate) condition_results_json: String,
...
pub(crate) fn status_from_parts(parts: StatusParts) -> Result<TargetStatus> {
    Ok(TargetStatus {
        ...
        render: parse_json(parts.render_json.as_deref().unwrap_or("{}"))?,
...
pub(crate) fn check_run_from_parts(parts: CheckRunParts) -> Result<CheckRun> {
    Ok(CheckRun {
        checked_at: parts.checked_at,
        matched: parts.matched.map(|value| value != 0),
        engine_used: parts.engine_used.and_then(|value| str_to_engine(&value)),
        price_cents: parts.price_cents,
        evidence: parse_json(&parts.evidence_json)?,
        condition_results: parse_json(&parts.condition_results_json)?,
        error: parts.error,
    })
}
