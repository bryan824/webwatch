...
use crate::{
    config::{CheckOutcome, CheckRun, Target, TargetStatus},
    error::{ParseStateSnafu, Result, SerializeStateSnafu},
};

use super::{
    check_run_from_parts, engine_to_str, status_from_parts, CheckRunParts, Persistence,
    StatusParts, CREATE_TABLES, DROP_TABLES, SCHEMA_VERSION, STATUS_SQL,
};
...
impl Persistence for DieselPersistence {
    async fn migrate(&self) -> Result<()> {
        ...
        spawn(move || {
            ...
            let version = sql_query("PRAGMA user_version")
                ...
                .unwrap_or(0);
            if version == 2 {
                sql_query("ALTER TABLE targets ADD COLUMN render_json TEXT NOT NULL DEFAULT '{}'")
                    .execute(conn)
                    .map_err(db_err)?;
    ...
    async fn ensure_target(&self, target: &Target) -> Result<()> {
        ...
        spawn(move || {
            ...
            let render_json = serde_json::to_string(&target.render).context(SerializeStateSnafu)?;
            let conditions_json =
                serde_json::to_string(&target.conditions).context(SerializeStateSnafu)?;
            sql_query("INSERT INTO targets (id, name, url, enabled, interval_secs, render_json, conditions_json, updated_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8) ON CONFLICT(id) DO UPDATE SET name = excluded.name, url = excluded.url, enabled = excluded.enabled, interval_secs = excluded.interval_secs, render_json = excluded.render_json, conditions_json = excluded.conditions_json, updated_at = excluded.updated_at")
    ...
    async fn list_targets(&self) -> Result<Vec<Target>> {
        ...
        spawn(move || {
            let conn = &mut conn(&pool)?;
            sql_query("SELECT id, name, url, enabled, interval_secs, render_json, conditions_json FROM targets ORDER BY id")
                .load::<TargetRow>(conn)
                .map_err(db_err)?
                .into_iter()
                .map(target_from_row)
    ...
    async fn recent_checks(&self, target_id: &str, limit: usize) -> Result<Vec<CheckRun>> {
        let pool = self.pool.clone();
        let target_id = target_id.to_string();
        ...
        spawn(move || {
            ...
            let conn = &mut conn(&pool)?;
            sql_query("SELECT checked_at, matched, engine_used, price_cents, evidence_json, condition_results_json, error FROM checks WHERE target_id = ?1 ORDER BY checked_at DESC, id DESC LIMIT ?2")
                .bind::<Text, _>(&target_id)
                .bind::<BigInt, _>(limit)
                .load::<CheckRunRow>(conn)
                .map_err(db_err)?
                .into_iter()
                .map(check_run_from_row)
                .collect()
        })
        .await
    }
...
fn target_from_row(row: TargetRow) -> Result<Target> {
    let render = serde_json::from_str(&row.render_json).context(ParseStateSnafu)?;
...
fn check_run_from_row(row: CheckRunRow) -> Result<CheckRun> {
    check_run_from_parts(CheckRunParts {
