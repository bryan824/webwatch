...
pub async fn check_with_http(
    ...
    evaluate_document(target, EngineUsed::Http, &body, &final_url)
}
...
pub fn evaluate_document(
    ...
    for condition in &target.conditions {
        let result = evaluate_condition(condition, &document, &page_text, body, &target.url, final_url)?;
...
fn evaluate_condition(
    ...
    let base_matched = match &condition.rule {
        ...
        ConditionRule::Redirect { negate } => {
            let redirected = !urls_equivalent(target_url, final_url);
            if redirected {
                evidence.push(format!(
                    "request to '{target_url}' redirected to '{final_url}'"
                ));
            } else if *negate {
                evidence.push(format!("request to '{target_url}' was not redirected"));
            }
            redirected
        }
...
mod tests {
    ...
    use crate::config::{Condition, ConditionRule, EngineUsed, RenderPlan, Target};
    ...
    #[test]
    fn url_unchanged_matches_when_not_redirected() {
        let condition = Condition {
            id: Some("in-stock".to_string()),
            rule: ConditionRule::Redirect { negate: true },
        };
        let outcome = evaluate_document(
            target(condition),
            EngineUsed::Http,
    ...
    #[test]
    fn url_unchanged_does_not_match_when_redirected() {
        let condition = Condition {
            id: Some("in-stock".to_string()),
            rule: ConditionRule::Redirect { negate: true },
        };
