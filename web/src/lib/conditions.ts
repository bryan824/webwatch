...
export type Subject = 'text' | 'element' | 'value' | 'price' | 'redirect';
export type LocatorType = 'page' | 'css';
...
export const SUBJECTS: SubjectDef[] = [
  ...
  },
  {
    ...
    label: 'Page redirect',
    glyph: '→',
    help: 'Check whether visiting this URL redirects elsewhere — e.g. a product URL falling back to a category page when out of stock.',
    ops: [{ id: 'redirects', label: 'redirects away' }],
    value: null,
    locator: 'page',
  },
];
...
export function coerceForSubject(cond: Condition, subjectId: Subject): Condition {
  ...
  return { ...cond, subject: subjectId, op, locator };
}
...
export function describeCondition(c: Condition): string {
  ...
  if (c.subject === 'text') core = `the page contains "${c.value || '…'}"`;
    ...
    core = `${where} redirects elsewhere`;
...
function flipDescription(core: string): string {
  return core
    ...
    .replace('redirects elsewhere', "doesn't redirect (stays on the same page)");
}

function locatorText(locator: Locator): string {
  if (locator.type === 'page') return 'the page';
  if (!locator.query) return 'a selector';
...
const WIRE_MAP: Record<string, ConditionWireKind> = {
  ...
  'redirect|redirects|false': 'url_redirects',
  'redirect|redirects|true': 'url_unchanged',
};
...
export function toWire(c: Condition): ConditionInput {
  ...
  const input: ConditionInput = { id: c.cid, kind };
...
const REVERSE_MAP: Record<ConditionWireKind, { subject: Subject; op: string; negate: boolean }> = {
  ...
  url_redirects: { subject: 'redirect', op: 'redirects', negate: false },
  url_unchanged: { subject: 'redirect', op: 'redirects', negate: true },
};
...
export function fromWire(input: ConditionInput): Condition {
  ...
  if (input.id) seed.cid = input.id;
  return blankCondition(seed);
}

const UNSUPPORTED_OPS = new Set<string>();
