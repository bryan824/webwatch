...
pub async fn send_test_message(client: &reqwest::Client, config: &AppConfig) -> Result<()> {
    send_message(
        client,
        config,
        &DiscordMessage {
            content: "✅ webwatch Discord webhook test succeeded".to_string(),
            embeds: vec![],
        },
    )
    .await
...
mod tests {
    ...
    use crate::config::{
        CheckOutcome, ConditionKind, ConditionResult, EngineUsed, RenderPlan, Target, TargetStatus,
    };
