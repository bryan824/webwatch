...
use webwatch::{
    ...
    config::{Condition, ConditionRule, RenderPlan, RenderPolicy, Target},
    db,
};
...
#[tokio::test]
async fn import_targets_round_trips_render_policy() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir
        .path()
        .join(format!("render-policy-{}.sqlite3", db::backend_name()));
    let persistence = db::connect(path.to_str().expect("utf8 path"))
        .await
        .expect("connect");
    persistence.migrate().await.expect("migrate");
    ...
    target.render.policy = RenderPolicy::RenderFirst;
    target.render.fingerprint_seed = Some("bestbuy-airtag-open-box".to_string());
    target.render.wait_ms = Some(3000);

    persistence
        .import_targets(std::slice::from_ref(&target))
        .await
        .expect("import target");
    ...
    let stored = persistence
        ...
        .expect("target");
    assert_eq!(stored.render, target.render);
    ...
    let status = persistence
        ...
        .expect("status")
        .expect("target status");
    assert_eq!(status.render, target.render);
}
