...
impl Scheduler {
    ...
    pub fn renderer(&self) -> RendererService {
        self.renderer.clone()
    }
    ...
    pub async fn stats(&self) -> SchedulerStats {
        SchedulerStats {
            running_targets: self.inner.lock().await.tasks.len(),
            renderer_available: self.renderer.is_available(),
        }
    }
    ...
    fn spawn_target(&self, target: Target) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            if let Err(error) = run_target_loop(config, db, client, renderer, target).await {
...
async fn run_target_loop(
    ...
    loop {
        run_once(&config, db.as_ref(), &client, &renderer, target.clone()).await;
...
async fn run_once(
    ...
    match monitor::run_check(config, db, client, renderer, target).await {
